...
/**
 * Hybrid Clock Widget Provider
 * Uses native AnalogClock view for smooth automatic updates
 * Displays:
 * - Squircle (rounded square) dial with tick marks
 * - Hollow/outline hour hand
 * - Solid white minute hand
 * - Red second hand with oval near tip
 * Opens Clock app on tap
 */
class HybridClockWidgetProvider : AppWidgetProvider() {
    ...
    companion object {
        internal fun updateWidget(
            ...
            val views = RemoteViews(context.packageName, R.layout.hybrid_clock_widget)
